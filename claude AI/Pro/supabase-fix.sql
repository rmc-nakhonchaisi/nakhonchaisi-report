-- ============================================================
-- แก้ error "policy already exists" + เช็คว่าข้อมูลครบไหม
-- รัน SQL นี้แทน supabase-setup.sql
-- ============================================================

-- 1. ลบ policies เก่าออกก่อน
DROP POLICY IF EXISTS "public read" ON production_monthly;
DROP POLICY IF EXISTS "public read" ON quality_decade;
DROP POLICY IF EXISTS "public read" ON cancel_causes;
DROP POLICY IF EXISTS "public read" ON mix_targets;
DROP POLICY IF EXISTS "public read" ON capacity_config;
...
-- 3. ล้างข้อมูลเก่าแล้ว insert ใหม่สะอาด
TRUNCATE production_monthly, quality_decade, cancel_causes, mix_targets, capacity_config
...
-- 6. production_monthly ปี 2569 (จริง)
...
-- 7. production_monthly ปี 2568 (ตัวอย่าง)
INSERT INTO production_monthly
  (year, month, order_qty, produced, cancel, days, per_day, transport, stock, raft_order, raft_produced, raft_cancel, is_sample)
VALUES
...
-- 9. cancel_causes ปี 2569
INSERT INTO cancel_causes (year, month, cause_index, cause_label, count) VALUES
  (2569,1,0,'แพล้นเสีย',3),(2569,1,1,'ที่ดึงหินทรายเสีย',0),(2569,1,2,'เครนเสีย',0),
  (2569,1,3,'รถไฟขนเสาเข็มเสีย',0),(2569,1,4,'รถไฟขนคอนกรีตเสีย',1),(2569,1,5,'รถขนส่งเข้าเยอะ',0),
  (2569,1,6,'ผู้รับเหมาผลิตไม่ได้',3),(2569,1,7,'สต็อคเต็ม',2),(2569,1,8,'ซ่อมแพผลิต',1),(2569,1,9,'ฝ่ายผลิตยกเลิกเอง',0),
  (2569,2,0,'แพล้นเสีย',1),(2569,2,1,'ที่ดึงหินทรายเสีย',0),(2569,2,2,'เครนเสีย',0),
  (2569,2,3,'รถไฟขนเสาเข็มเสีย',0),(2569,2,4,'รถไฟขนคอนกรีตเสีย',0),(2569,2,5,'รถขนส่งเข้าเยอะ',1),
  (2569,2,6,'ผู้รับเหมาผลิตไม่ได้',0),(2569,2,7,'สต็อคเต็ม',0),(2569,2,8,'ซ่อมแพผลิต',0),(2569,2,9,'ฝ่ายผลิตยกเลิกเอง',0),
  (2569,3,0,'แพล้นเสีย',9),(2569,3,1,'ที่ดึงหินทรายเสีย',5),(2569,3,2,'เครนเสีย',0),
  (2569,3,3,'รถไฟขนเสาเข็มเสีย',1),(2569,3,4,'รถไฟขนคอนกรีตเสีย',0),(2569,3,5,'รถขนส่งเข้าเยอะ',1),
  (2569,3,6,'ผู้รับเหมาผลิตไม่ได้',0),(2569,3,7,'สต็อคเต็ม',0),(2569,3,8,'ซ่อมแพผลิต',0),(2569,3,9,'ฝ่ายผลิตยกเลิกเอง',1),
  (2569,4,0,'แพล้นเสีย',6),(2569,4,1,'ที่ดึงหินทรายเสีย',4),(2569,4,2,'เครนเสีย',0),
  (2569,4,3,'รถไฟขนเสาเข็มเสีย',2),(2569,4,4,'รถไฟขนคอนกรีตเสีย',0),(2569,4,5,'รถขนส่งเข้าเยอะ',1),
  (2569,4,6,'ผู้รับเหมาผลิตไม่ได้',0),(2569,4,7,'สต็อคเต็ม',1),(2569,4,8,'ซ่อมแพผลิต',0),(2569,4,9,'ฝ่ายผลิตยกเลิกเอง',0);

-- 10. ตรวจสอบผล
SELECT 'production_monthly' as tbl, count(*) FROM production_monthly
UNION ALL SELECT 'quality_decade',  count(*) FROM quality_decade
UNION ALL SELECT 'cancel_causes',   count(*) FROM cancel_causes
